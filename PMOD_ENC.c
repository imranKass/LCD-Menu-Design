...
{
//Enable the clock to the Port D by setting the R3 in the RCGCGPIO register
	
	SYSCTL -> RCGCGPIO |= 0x08;
	//confiure the PD3, PD2, PD1, and PD0 pins as input 
	// clears bits 3 to 0 in the DIR register
	GPIOD -> DIR &= ~PMOD_ENC_ALL_PINS_MASK;
	
	//configure the PD3, PD2, PD1, and PD0 pins as input
	// by clearing the bits 3 to 0 in the AFSEL register
	
	GPIOD -> AFSEL &= ~PMOD_ENC_ALL_PINS_MASK;
	
	//Enable the digital functionality for the PD3,PD2,PD1, and PD0 pins
	//sets bits 3 to 0 in the DEN register
...
int PMOD_ENC_Get_Rotation(uint8_t state, uint8_t last_state)
{
	...
	//clcockwise. Return "-1" if the direction is counter clockwise. Otherwise,
