...
{
	//Enable the clock to PORT A by setting the R0 bit in the RCGCGPIO register
	
	SYSCTL-> RCGCGPIO |= 0x01;
	
	//COnfigures PA5, PA4, PA3, and PA2 pins as output by setitng bits 5 to 2 in the DIR register
	GPIOA->DIR |= 0x3C;
	
	//configure the PA5,PA4,PA3, and PA2 pins as output by clearing bits 5 to 2 in the DIr register
	GPIOA->AFSEL &= ~0x3C;
	
	// Enable the digital functionality for the PA5,PA4,PA3, and PA2 pins
	//Sets bits 5 to 2 in the AFSEL register
	GPIOA -> DEN |= 0x3c;
	
	//Initialize output of the PA5, PA4, PA3 and PA2 pins ot zero
	// clears bits 5 to 2 in the DATA register
	GPIOA-> DATA &= ~ 0x3C;
	
	//ENables Port C
	//R2 bit in the RCGCGPIO register
	SYSCTL-> RCGCGPIO |= 0x04;
	
	//configure the PC6 pin as output by setting Bit 6 in the DIR register
	GPIOC -> DIR |= 0x40;
	
	//Configure the PC6 pin to function as GPIO pin
	//clearing the bit 6 in the afsel register
	GPIOC -> AFSEL &= ~0x40;
	
	//Enable the digital functionality for the PA6 pin
	//setting bit 6 in DEN register
	GPIOC -> DEN |= 0x40;
	
	//Initialize the output of the PC6 pin to 0
	//clears bit 6 in data register
	GPIOC -> DATA &= ~ 0x40;
	
	//Enable the clock to Port E by setting R4 in the RCGCGPIO register
	SYSCTL -> RCGCGPIO |= 0x10;
	
	// configure PE0 pin as output by setting Bit 0 in the DIR register
	GPIOE -> DIR |= 0x01;
	
	//configure PE0 pin  to function as GPIO pin
	GPIOE -> AFSEL &= ~0x01;
	
	//// enable digital functionality for the PE0 pin
	//Sets bit 0 in register
	GPIOE -> DEN |= 0x01;
	
	// initialize the output of the PE0 pin to zero
	//clears bit 0 in data register
...
{
	//ensures that the output of the PC6 pin is zero before sending a short pulse	
	GPIOC -> DATA &= ~0x40;
	SysTick_Delay1us(1);
	
	//Output a short pulse on the PC6 pin by setting bit 6
	//in data register high and clearing it after 1 us
	//minimum time for the enable puls width must be at least greater than 450 ns
...
{
// set the upper nibble of data on data pins (PA2-PA5)
	GPIOA -> DATA  |= (data & 0xF0)>> 0x2;
	
	//sets or clear the register select (RS) pin based on the control flag 
	...
	// clears the LCD data lines (PA2-PA5) and provide a 1 ms delay
	GPIOA -> DATA &= ~0x3C;
...
void EduBase_LCD_Init(void)
{
	...
	// transmit a return home command to set the DDRAM address to 0 and to return display from shifted to original position
